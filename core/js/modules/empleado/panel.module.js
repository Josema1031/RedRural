...
const incFotoEvidencia = document.getElementById("incFotoEvidencia");
const incPreviewEvidencia = document.getElementById("incPreviewEvidencia");
const incImgPreviewEvidencia = document.getElementById("incImgPreviewEvidencia");
...
if (!empleadoDni || !productorId) {
  ...
  function limpiarPreviewIncidenciaEmpleado() {
    if (incImgPreviewEvidencia) incImgPreviewEvidencia.src = "";
    if (incPreviewEvidencia) incPreviewEvidencia.style.display = "none";
  }

  function mostrarPreviewIncidenciaEmpleado(file) {
    if (!file || !incImgPreviewEvidencia || !incPreviewEvidencia) return;

    const reader = new FileReader();
    reader.onload = (e) => {
      incImgPreviewEvidencia.src = e.target.result;
      incPreviewEvidencia.style.display = "block";
    };
    reader.readAsDataURL(file);
  }

  async function subirFotoIncidenciaEmpleado(file, productorId, empleadoDni) {
    if (!file) return "";

    const nombreSeguro = file.name.replace(/\s+/g, "_");
    const timestamp = Date.now();
    const carpetaUsuario = productorId || empleadoDni || "sin_uid";
    const ruta = `sisg-evidencias/${carpetaUsuario}/${timestamp}_${nombreSeguro}`;
  ...
  incFotoEvidencia?.addEventListener("change", (e) => {
    const file = e.target.files?.[0];
    if (file) {
      mostrarPreviewIncidenciaEmpleado(file);
    } else {
      limpiarPreviewIncidenciaEmpleado();
    }
  });
  ...
  try {
    ...
    const archivoFoto = incFotoEvidencia?.files?.[0] || null;
    ...
    if (archivoFoto) {
      evidenciaUrl = await subirFotoIncidenciaEmpleado(archivoFoto, productorId, empleadoDni);
    }
    ...
    await addDoc(collection(db, "seguridad_ganadera_incidencias"), {
      ...
      lat,
      lng,
      evidencia: archivoFoto ? archivoFoto.name : "",
      evidenciaUrl,
    ...
    if (incFotoEvidencia) incFotoEvidencia.value = "";
    estadoUbicacionInc.textContent = "";
    limpiarPreviewIncidenciaEmpleado();

    setMsgInc(
      archivoFoto
        ? "✅ Incidencia guardada correctamente con foto."
        : "✅ Incidencia guardada correctamente."
    );
  } catch (e) {
    console.error("Error guardando incidencia del empleado:", e);

    if (String(e?.code || "").includes("storage/unauthorized")) {
      setMsgInc("❌ No tenés permisos en Storage para subir la foto.", false);
      return;
    }
